...
def update_all_symbols(backup_db=False):
    ...
    # Process symbols with rate limiting (max 20 calls per second)
    for i, symbol_data in enumerate(all_symbols):
        ...
        try:
            ...
            if RATE_LIMIT:
                # Add rate limiting - sleep to ensure max 20 calls per second
                if (i + 1) % 20 == 0:
                    time.sleep(1)
        except Exception as e:
...
def parquet_plot_ohlc(df, symbol, interval='OneMinute', n_days=None):
    ...
    -----------
    df : pandas.DataFrame
        DataFrame with MultiIndex (symbol, interval, start) or (symbol, start)
    symbol : str
    ...
    """
    try:
        # Check if the DataFrame has a 3-level or 2-level MultiIndex
        if df.index.nlevels == 3:
            ...
            # Filter DataFrame for the specific symbol
            symbol_data = df.xs(symbol, level=0)
            # Filter by interval if it's a column
            if 'interval' in df.columns and interval:
                symbol_data = symbol_data[symbol_data['interval'] == interval]
        else:
            raise ValueError("DataFrame index structure not supported")
        ...
        if 'industrySector' in symbol_data.columns:
            industry = symbol_data['industrySector'].iloc[0] if not symbol_data.empty else 'Unknown'
            ax.annotate(f'Industry: {industry}', xy=(0.02, 0.95), xycoords='axes fraction', 
...
def prepare_parquet_autogluon(update_db=True):
    ...
    print(colored(f"Saving combined data to {DATA_DIR}/parquet/{time.strftime('%Y%m%d')}_AG_combined.parquet", "green"))
